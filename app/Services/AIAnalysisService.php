...
class AIAnalysisService
{
    ...
    public function analyzeMealPhoto($photoPath, $portionSize, $profile = null)
    {
        try {
            Log::info('Starting meal photo analysis', [
                'photo_path' => $photoPath,
                'portion_size' => $portionSize,
                'profile' => $profile ? (array)$profile : 'Not available'
            ]);

            // Validate inputs
            if (!is_numeric($portionSize) || $portionSize <= 0) {
                Log::warning('Invalid portion size', ['portion_size' => $portionSize]);
                throw new \Exception('Portion size must be a positive number');
            }
            ...
            try {
                $detection = $this->detectItems($base64Image, $portionSize);
                ...
                if ($isNonFood && empty($foodItems)) {
                    Log::warning('Non-food items detected, no food items found', [
                        ...
                        'user_id' => $profile ? $profile->user_id ?? 'unknown' : 'No profile',
                        'photo_path' => $photoPath
                    ]);
                    return [
                        'food' => 'Unknown',
            ...
            try {
                $macronutrients = $this->formatMacronutrients($nutrients);
                $recommendations = $this->generateRecommendations($nutrients, $calories, $profile ?? (object)['daily_calories' => 2000, 'health_conditions' => [], 'goal' => '']);
            ...
            $analysis = [
                ...
                'health_warnings' => $this->checkHealthConditions($foodItems, $nutrients, $profile ?? (object)['health_conditions' => []]),
                'source' => $source,
    ...
    protected function detectItems($base64Image, $portionSize)
    {
        $maxRetries = 3;
        $pythonPath = env('PYTHON_PATH', '/home/apexgnoa/miniconda3/bin/python3.8');
        $pythonScript = '/home/apexgnoa/bincone.apexjets.org/huggingface_predict.py';
        ...
        $escapedPortionSize = escapeshellarg($portionSize);

        for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
            try {
                $command = "{$pythonPath} {$pythonScript} {$escapedTempImageFile} {$escapedToken} {$escapedPortionSize} 2>&1";
    ...
    {
        $recommendations = [];
        $dailyGoal = $profile ? ($profile->daily_calories ?? 2000) : 2000;
        ...
        if ($nutrients['sodium'] > 1000 && in_array('hypertension', $profile ? ($profile->health_conditions ?? []) : [])) {
            $recommendations[] = 'High sodium content detected. Consider low-sodium alternatives.';
        } elseif ($nutrients['sodium'] <= 1000 && in_array('hypertension', $profile ? ($profile->health_conditions ?? []) : [])) {
            $recommendations[] = 'Sodium content is suitable for hypertension management.';
        }

        if (($profile ? $profile->goal : '') === 'weight_gain' && $calories < $mealCalorieTarget) {
    ...
    {
        $warnings = [];
        $healthConditions = $profile ? ($profile->health_conditions ?? []) : [];
    ...
    public function analyzeLeftoverPhoto($photoPath, $meal, $profile = null)
    {
        try {
            ...
            $detection = $this->detectItems($base64Image, $meal->portion_size ?? 100);
            ...
            if ($meal->portion_size && $meal->portion_size > 0) {
                $portionFactor = $meal->portion_size / 100;
                foreach ($nutrients as $nutrient => $value) {
