...
def predict(image_base64_path, hf_token=None, portion_size=None):
    try:
        ...
        # Scale nutrients and calories based on user-provided portion_size
        if portion_size and portion_size != 100.0:
            portion_factor = portion_size / 100.0
            nutrients = {k: v * portion_factor for k, v in nutrients.items()}
        ...
        if not ingredients and top_food != "unknown":
            ingredients = [{"name": top_food, "probability": food_items.get(top_food, 0), "subclasses": ["dough", "tomato sauce", "cheese"] if top_food.lower() == "pizza" else []}]

        # Fallback micronutrients
        if not micronutrients:
            micronutrients = {"vitamin_c": 19.38, "calcium": 324.04, "iron": 5.85} if top_food.lower() == "pizza" else {"vitamin_c": 0, "calcium": 0, "iron": 0}
...
if __name__ == "__main__":
    ...
    portion_size = float(sys.argv[3]) if len(sys.argv) > 3 and sys.argv[3] else None
